...
class AddEngineerFormWidget extends StatelessWidget {
  const AddEngineerFormWidget({super.key});

  @override
  Widget build(BuildContext context) {
    var cubit = EngineerCubit.get(context);
            ...
            BlocBuilder<EngineerCubit, EngineerState>(
              builder: (context, state) {
                return SizedBox(
                  ...
                  child: AppTextButton(
                    textButton: 'أضافة',
                    isLoading: state is AddEngineerLoadingState ? true : false,
                    onPressed: () {
  ...
  void validateThenNavigate(context) {
    if (EngineerCubit.get(context).formKey.currentState!.validate()) {
      EngineerCubit.get(context).addEngineer();
    }
  }
}
